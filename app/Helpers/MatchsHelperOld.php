...
class MatchsHelper
{
    ...
    static function getFromGuests($user_id, $event_id, $status = null, $order_by = null, $search = null): Collection
    {
        ...
        $query = Guest::select('users.*', 'user_profiles.work as work', 'user_profiles.height as height', 'guests.id as guests_id', 
        ...
        ->join('personal_filters', 'personal_filters.user_id', '=', 'users.id')//must have personal filter
        ...
        ->where('matches.user_two_id', null) //no already match one way
        ->where('matches.user_one_id', null) //no already match the other way
        ...
        $query->Where('personal_filters.mode', 'like', '%' . $interested_mode . '%');
        ...
        foreach($first_results as $r) {
            ...
            $counter++;
            $coincidences++; //1 to coincidences as they past the first filters
    ...
    static function getAll($user_id, $order_by = null, $search = null, $lat = null, $long = null): Collection
    { 
        $matches = new Collection();
        ...
        $query = User::select('users.*', 'user_profiles.work as work', 'user_profiles.height as height', 
